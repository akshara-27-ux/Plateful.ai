...
<div class="hero-box">
    ...
    <p style="color:#9DA7B3; max-width:700px;">
        Route commercial and event food excess directly to local volunteer networks and food banks.
    </p>
</div>
""", unsafe_allow_html=True)
...
user_input = st.chat_input("E.g., 'We have 40 meals left over in Indiranagar, Bangalore' or 'Excess food in Brooklyn'")
...
# ------------------------------------------------------------
# DISPATCH + VALIDATION + EXPORT WORKFLOW
# ------------------------------------------------------------
        ...
        with st.container(border=True):
            ...
            if submit_btn:
                if not name or not food or not quantity:
                    ...
                    qty_num = extract_portion_count(quantity)
                    impact = compute_impact(qty_num) if qty_num else compute_impact(st.session_state.last_portions)
                    csv_data = generate_csv_receipt(name, food, quantity, loc_str, org["name"], notes, impact)

                    st.success("Dispatch validated and compiled!")
